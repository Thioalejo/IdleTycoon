...
func _ready() -> void:
	GameManager._on_customer_request.connect(_on_customer_request)
	add_cashier()
...
func _on_customer_request(customer: Customer) -> void:
	...
	#Si no hay free cashier se deuvelve y ya
	if not free_cashiers:
		return
		
	#Si hay en la lista, entonces se crea una variable para escoger uno aleatoriamente
...
func _on_order_completed(cashier: Cashier)->void:
	var free_customer : Customer = counter_manager.get_first_available_custome()
	if free_customer:
